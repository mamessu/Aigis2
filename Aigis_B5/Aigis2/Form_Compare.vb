...
Public Class Form_Compare
    ...
    Private RadioButton() As System.Windows.Forms.RadioButton
    ...
    Private Sub FormLoad(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
        If Form_Default.TabControl1.SelectedIndex = 0 Then
            listview = Form_Default.ListView_Library
    ...
    Public Sub Combo()
        ...
        RadioButton = New System.Windows.Forms.RadioButton(Count) {}
        Dim NameArray As String()
        Dim i As Integer
        For i = 1 To Count
            'インスタンス作成            
            RadioButton(i) = New System.Windows.Forms.RadioButton
            PictureBox1.Controls.Add(RadioButton(i))
            RadioButton(i).Name = "Combobox_List" + i.ToString()
            RadioButton(i).Size = New Size(300, 40)
            RadioButton(i).Location = New Point(0, 70 + 40 * (i - 1))
            NameArray = Split(listview.SelectedItems(i - 1).Text, "/")
            Dim ItemNo As Integer = listview.SelectedItems(i - 1).Index
            RadioButton(i).Text = NameArray(1) & "#" & ItemNo
            RadioButton(i).CheckAlign = ContentAlignment.MiddleLeft
            RadioButton(i).ForeColor = Color.FromKnownColor(KnownColor.Red)
            RadioButton(i).BackColor = Color.Transparent
            'イベントハンドラに関連付け
            AddHandler Me.RadioButton(i).CheckedChanged, AddressOf Me.Check_Bottun
        Next
    End Sub

    '
    Private Sub Check_Bottun(sender As Object, e As EventArgs)
        If sender.checked = False Then
            Exit Sub
        End If
        Dim NameArray As String()
        NameArray = Split(sender.Text, "#")
        Dim No As Integer = NameArray(1)
        listview.SelectedItems.Clear()
        listview.Items(No).Selected = True
        Review(listview)
    End Sub
    ...
    Public Sub Shoshichange()
        '＝＝＝Json絡みの設定＝＝＝
        Dim Filename As String = listview.SelectedItems(0).ImageKey
        ...
        End If
        If listview.SelectedItems(0).BackColor = Color.Yellow Then
    ...
    Private Sub Review(sender As ListView)
        Shoshichange()
        If listview.SelectedItems.Count = 0 Then
            Exit Sub
        End If
    ...
    Private Sub Button_Hozon_Click(sender As Object, e As EventArgs) Handles Button_Hozon.Click
        ...
        Dim Filename As String = listview.SelectedItems(0).ImageKey
            ...
            If listview.SelectedItems(0).BackColor = Color.FromKnownColor(KnownColor.ActiveCaption) Then
                status = "済"
            Else
                status = "未"
            End If
            ...
            listview.SelectedItems(0).Text = status & "/【" & ComboBox_Syukkoyotei.Text & "】" & TextBox_Karimi.Text & "/" & strRegistTime
        JsonFile.Close()
    End Sub
